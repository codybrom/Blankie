...
4. **Test:** Ensure that your changes work correctly without causing issues in other areas. Blankie's unit tests (Swift Testing) cover core audio playback, preset management, the `Sound` model, and the app-data migration paths (UserDefaults and SwiftData into the app group). They run via the `Blankie.xctestplan` test plan — in Xcode with ⌘U, or on every pull request through the Tests CI workflow. The UI tests (`BlankieUITests/`, XCTest) are not part of the test plan and are run manually from Xcode. Please run the tests before submitting, and add new unit or UI tests for any behavior you change.
