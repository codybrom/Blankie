...
import SwiftUI
  ...
  extension MixerView {
    ...
    func moveItems(from source: IndexSet, to destination: Int) {
      debugLog("ListView: moveItems called - source: \(source), destination: \(destination)", .ui)

      // Check if we have a current preset (not default)
      if let preset = presetManager.currentPreset, !preset.isDefault {
        debugLog("ListView: Moving sounds in preset '\(preset.name)'", .ui)

        // Get the actual filtered sounds array that the list is displaying
        let displayedSounds = filteredSounds
        debugLog("ListView: Displayed sounds count: \(displayedSounds.count)", .ui)
        debugLog("ListView: Displayed sounds order: \(displayedSounds.map { $0.fileName })", .ui)
        ...
        for index in source where index < newOrder.count {
          debugLog("ListView: Moving '\(newOrder[index])' from index \(index) to \(destination)", .ui)
        }

        // Apply the move operation
        newOrder.move(fromOffsets: source, toOffset: destination)
        debugLog("ListView: New order after move: \(newOrder)", .ui)
        ...
        debugLog("ListView: Complete order being sent: \(completeOrder)", .ui)
        ...
        debugLog("ListView: UI refresh triggered", .ui)
      } else {
        // We're reordering the main sound grid (default preset or no preset)
        debugLog("ListView: Moving sounds in default view", .ui)
        ...
        debugLog("ListView: Complete default order being saved: \(completeOrder)", .ui)
        ...
        // Force UI refresh
        soundsUpdateTrigger += 1
        debugLog("ListView: UI refresh triggered for default view", .ui)
      }
    }
