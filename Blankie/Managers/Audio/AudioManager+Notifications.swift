...
extension AudioManager {
  func setupNotificationObservers() {
    ...
    func setupAudioSessionObservers() {
      ...
      debugLog("AudioManager: Setting up audio session observers on first playback", .audio)
      setupAudioInterruptionObserver()
      setupAudioRouteChangeObserver()
    ...
    func handleDidEnterBackground() {
      debugLog(
        "AudioManager: handleDidEnterBackground called - isGloballyPlaying: \(isGloballyPlaying)", .audio)
    ...
    func handleWillEnterForeground() {
      debugLog(
        "AudioManager: handleWillEnterForeground called - isGloballyPlaying: \(isGloballyPlaying)", .audio
      )
      ...
      Task { @MainActor in
        if isGloballyPlaying {
          debugLog("AudioManager: Refreshing media controls after foreground", .audio)
          setupMediaControls()
    ...
    private func handleInterruptionBegan() {
      debugLog("AudioManager: Audio interruption began - pausing playback", .audio)
      if isGloballyPlaying {
        Task { @MainActor in
    ...
    private func handleInterruptionEnded(userInfo: [AnyHashable: Any]) {
      if let optionsValue = userInfo[AVAudioSessionInterruptionOptionKey] as? UInt {
        ...
        if options.contains(.shouldResume) {
          debugLog(
            "AudioManager: Audio interruption ended with shouldResume flag - resuming playback", .audio)
          Task { @MainActor in
            self.setGlobalPlaybackState(true)
          }
        } else {
          debugLog("AudioManager: Audio interruption ended without shouldResume flag", .audio)
        }
      }
    }
    ...
    private func handleAudioRouteChange(_ notification: Notification) {
      ...
      case .oldDeviceUnavailable:
        debugLog("AudioManager: Audio route changed - old device unavailable", .audio)
        if isGloballyPlaying {
          Task { @MainActor in
            self.setGlobalPlaybackState(false)
          }
      ...
      case .newDeviceAvailable:
        debugLog("AudioManager: Audio route changed - new device available", .audio)
      default:
        break
      }
    }
  ...
  func handleAppTermination() {
    debugLog("AudioManager: App is terminating, cleaning up", .audio)
    cleanup()
  }
  ...
  func cleanup() {
    ...
    debugLog("AudioManager: Cleanup complete", .audio)
  }
}
