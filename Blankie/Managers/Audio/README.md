...
`publishInfo` is debounced 0.1 s, the same interval the 26 backend has always used, and the newest call wins. One preset switch publishes six times while its sounds settle (name first, then the sound summary, then the playing state), and every model write is a change the framework forwards. Pushed unbuffered from a run of remote next presses, that tripped MediaRemote's throttle ("Application exceeded audio metadata throttle limit"), after which the system drops updates, artwork included, and a card can arrive with its title but no still. `updatePlaybackState` is not debounced, so a remote pause still lands at once.

### Artwork, entities, commands

The system pulls artwork through the `Artwork(id:)` and `AnimatedArtwork(id:)` providers on its own schedule and caches by id, never asking for a cached id again. The still is decided in the same pass as the title, stored artwork included (a synchronous read of the SwiftData row or its app-group mirror), because the system reads an item's artwork once and a still that arrives later is never shown. `NowPlayingSessionMapping.artworkID` therefore encodes everything that changes the rendered image: the source (stored, cached file, bundled, soloed sound, drawn fallback) plus the accent color and the icons a montage fallback draws. Rebuilds are gated on the preset or soloed sound actually changing, because re-assigning artwork restarts the animated loop. The providers are `@Sendable` and capture only `Data` and `URL`.
