...
extension GlobalSettings {
  ...
  func setLanguage(_ newLanguage: Language) {
    guard newLanguage.code != language.code else {
      debugLog("Language not changed (already set to \(language.code))", .settings)
      return
    }

    debugLog("GlobalSettings: Changing language from \(language.code) to \(newLanguage.code)", .settings)
