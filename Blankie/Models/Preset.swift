...
struct Preset: Codable, Identifiable, Equatable {
  ...
  func validate() -> Bool {
    ...
    guard !soundStates.isEmpty else {
      debugLog("Preset: Must contain at least one sound", .presets)
      return false
    }
    ...
    for soundFileName in presetSounds where !availableSounds.contains(soundFileName) {
      debugLog("Preset: References non-existent sound '\(soundFileName)'", .presets)
      return false
    }

    // Validate volume ranges
    guard soundStates.allSatisfy({ $0.volume >= 0 && $0.volume <= 1 }) else {
      logError("Preset: Invalid volume range", .presets)
      return false
    }

    // Validate name
    guard !name.isEmpty else {
      debugLog("Preset: Empty name", .presets)
      return false
    }
